...
Control tmux auto-start behavior in `zsh.d/config.zsh`:

```bash
export TMUX_AUTOSTART_ENABLED=true        # Enable/disable
export TMUX_AUTOSTART_SESSION=default     # Session name
export TMUX_SKIP_SSH=false                # Skip in SSH
export TMUX_SKIP_IDE=true                 # Skip in IDEs (VSCode, etc)
export TMUX_SKIP_DESKTOP=true             # Skip in desktop environments
```

### Customize Tmux

Edit `tmux.local` for personal preferences:
- Color schemes
- Status bar content
- Key bindings
...
Edit the appropriate Makefile and add your installation target:
```makefile
mytool:  ## Install my custom tool
...
make profile    # Install ZSH, Tmux, Neovim
...
**Offline-First**: Works in air-gapped environments. Only tool installations require internet on first run.

**Modular**: Install only what you need. Each component is independent.

**Optimized**: Tmux config reduced from 1889 to 129 lines (93% reduction) while keeping all functionality.

**Best Practices**: Follows industry standards for Makefiles, shell configuration, and version control.
